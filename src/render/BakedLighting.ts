...
const restoreLegacyMaterialTuning = (
  sourceName: string,
  material: THREE.MeshStandardMaterial,
): void => {
  if (sourceName === 'aged-wallpaper') {
    material.color.setHex(0xf1efc9);
    material.emissive.setHex(0x5c5826);
    material.emissiveIntensity = 0.045;
    material.normalScale.set(0.22, 0.22);
  } else if (sourceName === 'aged-painted-plaster') {
    material.color.setHex(0xeee8af);
    material.emissive.setHex(0x5c5920);
    material.emissiveIntensity = 0.035;
  } else if (sourceName === 'damp-yellow-carpet') {
    material.color.setHex(0xfaf5cc);
    material.emissive.setHex(0x5d592f);
    material.emissiveIntensity = 0.035;
  } else if (sourceName === 'office-drop-ceiling') {
    material.color.setHex(0xddd080);
    material.emissive.setHex(0x827a32);
    material.emissiveIntensity = 0.04;
  } else if (sourceName === 'yellowed-baseboard') {
    material.color.setHex(0xbeb574);
    material.emissive.setHex(0x45411e);
    material.emissiveIntensity = 0.025;
  } else if (sourceName === 'white-biome-baseboard') {
    // This material inherited its emissive intensity from the yellow source
    // in the pre-e6d7e6a library before overriding only its colours.
    material.emissiveIntensity = 0.025;
  }
};
...
export const createBakedMaterialSet = (
  ...
  for (const [sourceMaterial, material] of [
    [source.wall, wall],
    [source.plaster, plaster],
    [source.floor, floor],
    [source.ceiling, ceiling],
    [source.baseboard, baseboard],
  ] as const) restoreLegacyMaterialTuning(sourceMaterial.name, material);
  const fixtureGlow = source.fixtureGlow.clone();
  fixtureGlow.toneMapped = false;
  fixtureGlow.fog = false;
  if (source.fixtureGlow.name === 'fluorescent-diffuser') {
    fixtureGlow.color.setHex(0xfffee6);
  } else if (source.fixtureGlow.name === 'red-biome-fluorescent-diffuser') {
    fixtureGlow.color.setHex(0xff1a0d);
  }
